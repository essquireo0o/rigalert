...
class SettingsPage(QWidget):
    ...
    def _setup_ui(self):
        ...
        self._quick_timeout = QDoubleSpinBox()
        self._quick_timeout.setRange(0.15, 5.0)
        self._quick_timeout.setDecimals(2)
        self._quick_timeout.setSingleStep(0.05)
        self._quick_timeout.setSuffix(" sec")
        self._quick_timeout.setFixedWidth(100)
        nf.addRow("Quick Probe Timeout:", self._quick_timeout)

        self._max_workers = QSpinBox()
        self._max_workers.setRange(1, 512)
        self._max_workers.setFixedWidth(100)
        nf.addRow("Scan Workers:", self._max_workers)

        self._full_scan_interval = QSpinBox()
        self._full_scan_interval.setRange(0, 1440)
        self._full_scan_interval.setSuffix(" min")
        self._full_scan_interval.setFixedWidth(100)
        nf.addRow("Full Scan Interval:", self._full_scan_interval)

        self._dead_host_backoff = QSpinBox()
        self._dead_host_backoff.setRange(0, 3600)
        self._dead_host_backoff.setSuffix(" sec")
        self._dead_host_backoff.setFixedWidth(100)
        nf.addRow("Dead Host Backoff:", self._dead_host_backoff)

        self._slow_response = QDoubleSpinBox()
        self._slow_response.setRange(0.1, 30.0)
        self._slow_response.setDecimals(1)
        self._slow_response.setSuffix(" sec")
        self._slow_response.setFixedWidth(100)
        nf.addRow("Slow Response Log:", self._slow_response)

        self._auto_refresh = QCheckBox("Auto-refresh miners in background")
        self._debug_scan = QCheckBox("Debug scan logging")
        nf.addRow("", self._auto_refresh)
        nf.addRow("", self._debug_scan)
    ...
    def _load(self):
        ...
        self._quick_timeout.setValue(getattr(cfg, "quick_scan_timeout", 0.8))
        self._max_workers.setValue(getattr(cfg, "max_scan_workers", 50))
        self._full_scan_interval.setValue(getattr(cfg, "full_scan_interval_minutes", 15))
        self._dead_host_backoff.setValue(getattr(cfg, "dead_host_backoff_seconds", 300))
        self._slow_response.setValue(getattr(cfg, "slow_response_seconds", 2.5))
        self._auto_refresh.setChecked(getattr(cfg, "auto_refresh_enabled", True))
        self._debug_scan.setChecked(getattr(cfg, "debug_scan_enabled", False))
    ...
    def _save(self):
        ...
        cfg.quick_scan_timeout = self._quick_timeout.value()
        cfg.max_scan_workers = self._max_workers.value()
        cfg.full_scan_interval_minutes = self._full_scan_interval.value()
        cfg.dead_host_backoff_seconds = self._dead_host_backoff.value()
        cfg.slow_response_seconds = self._slow_response.value()
        cfg.auto_refresh_enabled = self._auto_refresh.isChecked()
        cfg.debug_scan_enabled = self._debug_scan.isChecked()
