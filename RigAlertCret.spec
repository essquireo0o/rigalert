...
# Pre-configured build — credentials baked in via seed config
import os
...
a = Analysis(
    ...
    datas=[
        ('rigalert.ico', '.'),
        ('rigalert_preview.png', '.'),
        # Seed config — copied to EXE dir on first launch if no config exists
        ('rigalert_seed_config.json', '.'),
    ] + _tzdata_datas,
